...
from carla.recourse_methods import FeatureTweak
...
if __name__ == "__main__":
    ...
    model = MLModelCatalog(
        dataset, load_online=False, model_type="forest", backend="xgboost"
    )
    model.train(0.001, 1000, 64, force_train=True)

    # 3. Load recourse model with model specific hyperparameters
    hyperparameters = {}
    gs = FeatureTweak(model, hyperparameters)
